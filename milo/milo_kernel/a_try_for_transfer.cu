...
__global__ void Marlin_3bit_faster(
  ...
  constexpr int b_sh_stride = 32 * thread_n_blocks / 4; //128
  ...
  int b1_sh_wr = (b_sh_wr / 64) * 32 + b_sh_wr % 32;
  int b2_sh_wr = (b_sh_wr / 128) * 32 + b_sh_wr % 32;
  ...
  bool B2_sh_wr_pred[stages];
  bool B1_sh_wr_pred[stages];
  //bool B_sh_wr_pred[4];
  for (int pipe = 0 ;pipe < stages;pipe++){
    B2_sh_wr_pred[pipe] = (b_sh_wr / 32) % 4 == pipe;
    B1_sh_wr_pred[pipe] = (b_sh_wr / 32 + pipe) % 2;
    //B_sh_wr_pred[pipe] = B2_sh_wr_pred[pipe] ||  B1_sh_wr_pred[pipe];
  }
  const int4* B1_ptr[b_sh_wr_iters];
  const int4* B2_ptr[b_sh_wr_iters];
  int b_gl_rd = b_sh_stride * slice_col + b_gl_rd_delta_o * slice_row + b_gl_stride * (threadIdx.x/ b_sh_stride) ;
  int b_gl_rd_2 = b_gl_rd;
  b_gl_rd += 64 * ((threadIdx.x / 64) %2 )+ threadIdx.x % 32;
  b_gl_rd_2 += threadIdx.x % 32;
  for (int i = 0; i < b_sh_wr_iters; i++)
  {
  ...
  auto fetch_to_shared_faster = [&] (int pipe, int a_off, bool pred = true) {
    if (pred) {
      ...
      bool b_pred = (threadIdx.x / 32) %4 != (pipe +2) %4;
      #pragma unroll
      for (int i = 0; i < b_sh_wr_iters; i++) {
        
        int4* share_B;
        const int4* B_ptr = B1_ptr[i] ;
        if (B1_sh_wr_pred[pipe]){
          share_B = &sh_b1_stage[ (b_sh_wr_delta/2) * i + b1_sh_wr];
        }
        else if (B2_sh_wr_pred[pipe]){
          share_B =&sh_b2_stage[ (b_sh_wr_delta/4) * i + b2_sh_wr];
          B_ptr = B2_ptr[i];
        }  
        cp_async4_pred(share_B, B_ptr, b_pred);
        B1_ptr[i]+= b_gl_rd_delta_o/2;
        B2_ptr[i] += b_gl_rd_delta_o/4; 
      }
      
      //printf("blockIdx.x : %d, threadIdx.x %d, pipe %d, off %d",blockIdx.x, threadIdx.x, pipe,share_B - );
  ...
  while (slice_iters) {
    ...
    if (slice_iters == 0) {
      ...
      if (slice_iters) {
        ...
        //b_off += b_sh_stride - b_gl_rd_delta_o * k_tiles;
