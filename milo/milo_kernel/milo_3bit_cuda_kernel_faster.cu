...
>
__global__ void MiLo_3bit_faster(
...
auto matmul = [&] (int k_mod_2) {
  ...
  auto matmul_fast = [&] (int k_mod_2) {
    int b_quant, b_quant_shift, b_quant3;
    FragB frag_b0, frag_b1;
    b_quant = frag_b_quant[k_mod_2][0][0];
    b_quant_shift = b_quant >> 6;
    frag_b0 = dequant_faster(b_quant);
    scale(frag_b0, frag_s[k_mod_2][0], 0);
    frag_b1 = dequant_faster(b_quant_shift);
    b_quant3 = (b_quant >> 12) & 0xf000f;
    scale(frag_b1, frag_s[k_mod_2][0], 1);
    #pragma unroll
    for (int i = 0; i < thread_m_blocks; i++) {
      mma(frag_a[k_mod_2][i], frag_b0, frag_c[i][0][0]);
      mma(frag_a[k_mod_2][i], frag_b1, frag_c[i][0][1]);
    }

    b_quant = frag_b_quant[k_mod_2][0][1];
    b_quant_shift = b_quant >> 6;
    frag_b0 = dequant_faster(b_quant);
    scale(frag_b0, frag_s[k_mod_2][1], 0);
    frag_b1 = dequant_faster(b_quant_shift);
    b_quant3 |= (b_quant >> 8) & 0xf000f0;
    scale(frag_b1, frag_s[k_mod_2][1], 1);
    #pragma unroll
    for (int i = 0; i < thread_m_blocks; i++) {
      mma(frag_a[k_mod_2][i], frag_b0, frag_c[i][1][0]);
      mma(frag_a[k_mod_2][i], frag_b1, frag_c[i][1][1]);
    }

    b_quant = frag_b_quant[k_mod_2][1][0];
    b_quant_shift = b_quant >> 6;
    frag_b0 = dequant_faster(b_quant);
    scale(frag_b0, frag_s[k_mod_2][2], 0);
    frag_b1 = dequant_faster(b_quant_shift);
    b_quant3 |= (b_quant >> 4) & 0xf000f00 ;
    scale(frag_b1, frag_s[k_mod_2][2], 1);
    #pragma unroll
    for (int i = 0; i < thread_m_blocks; i++) {
      mma(frag_a[k_mod_2][i], frag_b0, frag_c[i][2][0]);
      mma(frag_a[k_mod_2][i], frag_b1, frag_c[i][2][1]);
    }
...
auto matmul_faster = [&] (int k_mod_2) {
    int b_quant[4], b_quant_shift[4];
    FragB frag_b0[4], frag_b1[4];
    // Precompute b_quant, b_quant_shift, and b_quant3 to reduce dependencies
    #pragma unroll
    for (int i = 0; i < 3; i++) {
        b_quant[i] = frag_b_quant[k_mod_2][i / 2][i % 2];
    }
    b_quant[3] = ((b_quant[0] >> 12) & 0xf000f) |((b_quant[1] >> 8) & 0xf000f0);
    #pragma unroll
    for (int i = 0; i < 3; i++) {
        b_quant_shift[i] = b_quant[i] >> 6;
    }
    b_quant_shift[3] = ((b_quant[1] >> 14) & 0x30003) |((b_quant[2] >> 4) & 0xf000f00);
    // Precompute frag_b0 and frag_b1 using dequant_faster
    #pragma unroll
    for (int i = 0; i < 3; i++) {
        frag_b0[i] = dequant_faster(b_quant[i]);
        //frag_b1[i] = dequant_faster(b_quant_shift[i]);
    }
    #pragma unroll
    for (int i = 0; i < 3; i++) {
        //frag_b0[i] = dequant_faster(b_quant[i]);
    ...
    // Perform scaling and MMA operations
    #pragma unroll
    for (int j = 0; j < 4; j++) {
        scale(frag_b0[j], frag_s[k_mod_2][j], 0);
        scale(frag_b1[j], frag_s[k_mod_2][j], 1);

        #pragma unroll
        for (int i = 0; i < thread_m_blocks; i++) {
            mma(frag_a[k_mod_2][i], frag_b0[j], frag_c[i][j][0]);
            mma(frag_a[k_mod_2][i], frag_b1[j], frag_c[i][j][1]);
        }
    }
  ...
  auto start_pipes = [&] () {
    ...
    start_pipes();
  auto start_pipes_without_share = [&] () {
    zero_accums();
    wait_for_stage();
    fetch_to_registers(0, 0);
    a_gl_rd += a_gl_rd_delta_o * (stages - 1);
  ...
  while (slice_iters) {
    ...
    if (slice_iters == 0) {
      ...
      if (slice_iters) {
        ...
        //if (threadIdx.x == 0 && blockIdx.x == 0) printf("start pipes \n");
        start_pipes();
        
        //start_pipes_without_share();
      }
    }
  }
}
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS) \
    ...
    cudaFuncSetAttribute( \
      MiLo_3bit_faster<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>, \
