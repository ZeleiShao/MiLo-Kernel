...
>
__global__ void MiLo_3bit_256_64_kernel(
...
/*
  ...
  for (int i = 0; i < b_sh_wr_iters; i++)
  {
    B1_ptr[i] = (I2*)B1 + b_gl_rd_delta_i * i + b_gl_rd;
    B2_ptr[i] = (int*)B2 + b_gl_rd_delta_i * i + b_gl_rd;
    //if (blockIdx.x  == 0 ){
  ...
  /*auto fetch_to_shared_naive = [&] (int pipe, int a_off, bool pred = true) {
    if (pred) {
...
__shared__ half2 deq2[64][32];
  ...
  auto matmul = [&] (int k) {
    ...
    unsigned int b_quant_0 = as_unsigned(frag_b_quant[k % 2][0][0]);
    unsigned int b_quant_1 = as_unsigned(frag_b_quant[k % 2][0][1]);
    unsigned int b_quant_2 = as_unsigned(frag_b_quant[k % 2][1][0]);
    ...
    for(int j = 0; j < 4; j++) {
      ...
      if (group_blocks != -1) { //TODO: Some bug here
...
auto matmul_fast = [&] (int k_mod_2) {
    int b_quant[4], b_quant_shift[4], b_quant3[4];
    ...
    for (int i = 0; i < 4; i++) {
        ...
        b_quant3[i] = (b_quant[i] >> (4 + i * 4)) & (0xf000f << i);
    }
    ...
    if (slice_iters == 0) {
      ...
      if (slice_iters) {
        ...
        for (int i = 0; i < b_sh_wr_iters; i++)
        {
          ...
          //B1_ptr[i] += (b_sh_stage - b_gl_rd_delta_o * k_tiles);
          //B2_ptr[i] += (b_sh_stage - b_gl_rd_delta_o * k_tiles);
        }
        if (slice_col == 0) {
          #pragma unroll
          for (int i = 0; i < b_sh_wr_iters; i++){
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS) \
    ...
    cudaFuncSetAttribute( \
      MiLo_3bit_256_64_kernel<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>, \
      cudaFuncAttributeMaxDynamicSharedMemorySize, \
      SHARED_MEM \
    ); \
    MiLo_3bit_256_64_kernel< \
