...
def mul(A, B, C, s, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    ...
    """
    milo_cuda.mul(A, B, C, s, workspace, thread_k, thread_n, sms, max_par)
...
def mul_3bit_faster(A, B1, B2, C, s, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    ...
    @max_par: maximum number of batch 64 problems to solve in parallel for large input sizes
    """
    milo_cuda.mul_3bit_faster(A, B1, B2, C, s, workspace, thread_k, thread_n, sms, max_par)
...
def mul_3bit_with_zero(A, B1, B2, C, s, zero, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    ...
    @max_par: maximum number of batch 64 problems to solve in parallel for large input sizes
    """
    milo_cuda.mul_3bit_with_zero(A, B1, B2, C, s, zero, workspace, thread_k, thread_n, sms, max_par)
...
def mul_3bit_256_64(A, B1, B2, C, s, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    ...
    @max_par: maximum number of batch 64 problems to solve in parallel for large input sizes
    """
    milo_cuda.mul_3bit_256_64(A, B1, B2, C, s, workspace, thread_k, thread_n, sms, max_par)

def mul_3bit_256_64_with_zero(A, B1, B2, C, s, z, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    """Marlin FP16xINT3 multiply; can be used within `torch.compile`.
    @A: `torch.half` input matrix of shape `(m, k)` in standard row-major layout
    @B: `torch.int` weight matrix of original shape `(k, n)` in Marlin format; see `Layer.pack()`
    @C: `torch.half` out matrix of shape `(m, n)` in standard row-major layout
    ...
    @max_par: maximum number of batch 64 problems to solve in parallel for large input sizes
    """
    milo_cuda.mul_3bit_256_64_with_zero(A, B1, B2, C, s, z, workspace, thread_k, thread_n, sms, max_par)

def mul_3bit_64_256_with_zero(A, B1, B2, C, s, z, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    """Marlin FP16xINT3 multiply; can be used within `torch.compile`.
    @A: `torch.half` input matrix of shape `(m, k)` in standard row-major layout
    @B: `torch.int` weight matrix of original shape `(k, n)` in Marlin format; see `Layer.pack()`
    @C: `torch.half` out matrix of shape `(m, n)` in standard row-major layout
    ...
    @max_par: maximum number of batch 64 problems to solve in parallel for large input sizes
    """
    milo_cuda.mul_3bit_64_256_with_zero(A, B1, B2, C, s, z, workspace, thread_k, thread_n, sms, max_par)
...
class Layer3bit(nn.Module):
    ...
    def __init__(self, infeatures, outfeatures, groupsize=64, sms = 108):
        ...
        global_reduce_64_256 = self.k // (thread_block_num * 64) + 2;
        global_reduce_256_64 = self.k // (thread_block_num * 256) + 2;
        if ((global_reduce_256_64 < global_reduce_64_256) or (self.n % 256 != 0)):
            self.tile_shape = 1
        if (self.k % 256 != 0):
            self.tile_shape = 0
...
class Layer3bitWithZero(nn.Module):
    ...
    def pack(self, linear, scales, zeros):
        ...
        @linear: fake-quantized `torch.nn.Linear` layer to convert (must be of type `torch.half`)
        @scales: corresponding quantization scales of shape `(infeatures, groups)`
        @tile_shape : 0 if (64, 256), 1 if (256, 64)
        """ 
        if linear.weight.dtype != torch.half:
            raise ValueError('Only `torch.half` weights are supported.')
        tile = 16
...
class Layer3bit_64_256_WithZero(nn.Module):
    """PyTorch compatible Marlin layer; 3-bit (symmetric grouped) linear layer without bias."""
    ...
    def forward(self, A):
        C = torch.empty(A.shape[:-1] + (self.s.shape[1],), dtype=A.dtype, device=A.device)
        mul_3bit_64_256_with_zero(A.view((-1, A.shape[-1])), self.B1, self.B2, C.view((-1, C.shape[-1])), self.s, self.z, self.workspace)
        return C
...
class Layer3bit_cutlass(nn.Module):
    def __init__(self, infeatures, outfeatures, groupsize=64):
        """Create an empty Marlin layer.
        @infeatures: number of input features (must be divisible by 128)
        @outfeatures: number of output features (must be divisible by 256)
    ...
    def forward(self, A):
        C = torch.empty(A.shape[:-1] + (self.s.shape[1],), dtype=A.dtype, device=A.device)
        mul_3bit_cutlass(A.view((-1, A.shape[-1])), self.B1, self.B2, C.view((-1, C.shape[-1])), self.s, self.workspace)
        return C
...
class Layer3bit256_64_with_zero(nn.Module):
    """PyTorch compatible Marlin layer; 3-bit (symmetric grouped) linear layer without bias."""
    def __init__(self, infeatures, outfeatures, groupsize=64):
        """Create an empty Marlin layer.
        @infeatures: number of input features (must be divisible by 128)
    ...
    def forward(self, A):
        C = torch.empty(A.shape[:-1] + (self.s.shape[1],), dtype=A.dtype, device=A.device)
        mul_3bit_256_64_with_zero(A.view((-1, A.shape[-1])), self.B1, self.B2, C.view((-1, C.shape[-1])), self.s, self.z, self.workspace)
        return C
...
def replace_linear_3bitFaster(module, name_filter=lambda n: True, groupsize=-1, name=''):
    ...
    """
    if isinstance(module, Layer3bitFaster):
        return
    for attr in dir(module):
        ...
        if isinstance(tmp, nn.Linear) and name_filter(name1):
            setattr(
                module, attr, Layer3bitFaster(tmp.in_features, tmp.out_features, groupsize=groupsize)
            )
    for name1, child in module.named_children():
        replace_linear_3bitFaster(child, name_filter, groupsize=groupsize, name=name + '.' + name1 if name != '' else name1)
